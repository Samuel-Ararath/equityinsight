...
export function AdvancedMarketChart({ candles, toolkit }: ChartProps) {
  ...
  useEffect(() => {
    ...
    main.timeScale().fitContent(); if (visibleRange) main.timeScale().setVisibleLogicalRange(visibleRange)
    ...
    const ro = new ResizeObserver(() => { if (mainRef.current) main.applyOptions({ width: mainRef.current.clientWidth }); charts.slice(1).forEach((chart, index) => { const element = [oscillatorRef.current, macdRef.current, volumeRef.current][index]; if (element) chart.applyOptions({ width: element.clientWidth }) }) }); ro.observe(mainRef.current)
    ...
    return () => { ro.disconnect(); main.unsubscribeCrosshairMove(onCrosshair); priceLines.forEach((remove) => remove()); cleanupSeries.forEach((remove) => remove()); charts.forEach((chart) => { try { chart.remove() } catch {} }) }
  }, [candles, toolkit])

  const activeCandle = tooltip ? candles[tooltip.index] : null
  return <div className="relative space-y-2"><div ref={mainRef} className="relative min-h-[480px] w-full" />{toolkit.rsi || toolkit.stochastic ? <Panel title="Momentum"><div ref={oscillatorRef} className="h-[190px] w-full" /></Panel> : null}{toolkit.macd ? <Panel title="MACD"><div ref={macdRef} className="h-[170px] w-full" /></Panel> : null}{toolkit.volume ? <Panel title="Volume"><div ref={volumeRef} className="h-[120px] w-full" /></Panel> : null}{activeCandle && tooltip && <div className="pointer-events-none absolute z-20 rounded-lg border border-primary/30 bg-background/95 px-3 py-2 text-xs shadow-xl" style={{ left: Math.min(Math.max(tooltip.x + 18, 8), 360), top: Math.max(tooltip.y - 42, 8) }}><div className="flex items-center gap-2 font-semibold"><span>{new Date(activeCandle.candle_time).toLocaleString('id-ID')}</span><span className="text-primary">{tooltip.index ? 'HH/HL/LH/LL' : ''}</span></div><div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 text-muted-foreground"><span>O <b className="text-foreground">{activeCandle.open}</b></span><span>H <b className="text-foreground">{activeCandle.high}</b></span><span>L <b className="text-foreground">{activeCandle.low}</b></span><span>C <b className="text-foreground">{activeCandle.close}</b></span></div></div>}{snapshot && null}</div>
}
